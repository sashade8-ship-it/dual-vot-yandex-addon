...
package app.dualvot.extension.youtube.settings.preference;
...
import app.dualvot.extension.youtube.patches.yandex.YandexVotApiClient;
import app.dualvot.extension.youtube.patches.yandex.YandexVotAuthWebViewDialog;
...
public class YandexVotOAuthPreference extends Preference implements Preference.OnPreferenceClickListener {
    ...
    private void updateUI() {
        if (isSignedIn()) {
            if (cachedDisplayName != null && !cachedDisplayName.isEmpty()) {
                setSummary(str("dualvot_yandex_oauth_signed_in_summary", cachedDisplayName));
            } else {
                setSummary(str("dualvot_yandex_oauth_signed_in_summary", "Yandex"));
                // Try to load profile info in the background
                loadProfileAsync();
            }
        } else {
            cachedDisplayName = null;
            setSummary(str("dualvot_yandex_oauth_not_signed_in_summary"));
        }
    }
    ...
    private void showAuthMethodDialog() {
        ...
        CustomDialog.create(
                context,
                str("dualvot_yandex_oauth_auth_method_title"),
                str("dualvot_yandex_oauth_auth_method_message"),
                null,
                str("dualvot_yandex_oauth_sign_in_button"),
                () -> openWebViewAuth(context),
                () -> { /* Cancel — do nothing */ },
                str("dualvot_yandex_oauth_enter_manually_button"),
    ...
    private void showAccountManagementDialog() {
        ...
        String message = cachedDisplayName != null
                ? cachedDisplayName
                : str("dualvot_yandex_oauth_signed_in_summary", "Yandex");

        CustomDialog.create(
                context,
                str("dualvot_yandex_oauth_account_management_title"),
                message,
                null,
                str("dualvot_yandex_oauth_sign_out_button"),
                () -> signOut(),
                () -> { /* Cancel — do nothing */ },
                str("dualvot_yandex_oauth_switch_token_button"),
                () -> showAuthMethodDialog(),
                false
        ).first.show();
    }
    ...
    private void showManualTokenDialog(Context context) {
        ...
        CustomDialog.create(
                context,
                str("dualvot_yandex_oauth_enter_token_title"),
                null,
                editText,
                null,
                () -> {
        ...
        Utils.runOnBackgroundThread(() -> {
            ...
            if (!YandexVotApiClient.isValidOAuthToken(token)) {
                Utils.runOnMainThread(() -> {
                    Utils.showToastLong(str("dualvot_yandex_oauth_invalid_token"));
                });
                return;
            }

...
public final class YandexVotOriginalVolumePatch {
    ...
    private static float applyMultiplier(float volume, int volumePercent) {
        // A pending request or a prepared-but-paused player must not duck the
        // original track.  The translated MediaPlayer itself is the only
        // authoritative signal that translated audio actually started.
        if (!YandexVoiceOverTranslationPatch.isTranslationAudioPlaying()) {
            return volume;
        }
    ...
    /**
     * Restores the last observed original-player volume after a manual stop,
     * an error, a video change, or coordinator invalidation.
     */
    public static void clearAudioMultiplier() {
        ...
        float base = lastBaseVolume;
        if (Float.isNaN(base)) base = 1.0f;
        base = Math.max(0f, Math.min(base, 1f));
        applyingNow = true;
        try {
            audioTrack.setVolume(base);
        } catch (Exception ignored) {
            // The platform can release an AudioTrack between the reference read
            // and this best-effort restoration.
        } finally {
            applyingNow = false;
        }
    }

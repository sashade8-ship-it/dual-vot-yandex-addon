...
final class YandexVotAudioDownloader {
    ...
    static boolean downloadAndSend(
        ...
        try {
            ...
            Logger.printException(() -> "Yandex VOT audio upload failed ("
    ...
    private static AudioFormatInfo fetchAudioFormat(String videoId) throws Exception {
        // The API-v1 base exposes a compact PlayerResponse whose Format only
        // guarantees URL and signature fields.  It intentionally does not
        // expose itag, MIME type, or bitrate, so selecting from that cached
        // response could upload a video stream as audio.  Resolve a dedicated
        // audio-only ANDROID_VR stream instead; this is the same resilient
        // fallback used when no compatible cached stream is available.

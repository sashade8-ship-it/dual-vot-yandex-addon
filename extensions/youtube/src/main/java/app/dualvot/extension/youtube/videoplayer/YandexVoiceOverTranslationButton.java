...
package app.dualvot.extension.youtube.videoplayer;
...
import app.dualvot.extension.youtube.patches.yandex.YandexVotAddOn;
import app.dualvot.extension.youtube.patches.yandex.YandexVoiceOverTranslationPatch;
import app.dualvot.extension.youtube.patches.yandex.YandexVotBottomSheet;
...
public final class YandexVoiceOverTranslationButton {
    ...
    public static void initializeButton(View controlsView) {
        try {
            ...
            YandexVotAddOn.addOnEngineStateChangeCallback(STATE_REFRESH_CALLBACK);
            ImageView hostButton = PlayerOverlayButton.addButton(
                    controlsView,
            ...
            YandexCountdownButton button = createCountdownOverlay(hostButton);
    ...
    public static void initializeLegacyButton(View controlsView) {
        try {
            ...
            // The host owns the legacy layout slots.  Claim one through its
            // public API instead of injecting an add-on-specific view id.
            legacy = AddOnApi.createLegacyButton(
                    YandexVotAddOn.ENGINE_ID,
                    controlsView,
    ...
    private static void refreshActivatedState() {
        ...
        try {
            boolean active = YandexVotAddOn.isYandexActive();
    ...
    /**
     * API v1 exposes the host overlay button factory, whose returned ImageView
     * keeps all host-managed placement and spacing.  The countdown/ring view
     * is layered over that anchor and follows it on every pre-draw pass, so
     * this add-on retains its custom visual state without relying on a
     * parent-only PlayerOverlayButton overload.
     */
    @Nullable
    private static YandexCountdownButton createCountdownOverlay(
            @Nullable ImageView hostButton
    ) {
        if (hostButton == null || !(hostButton.getParent() instanceof ViewGroup parent)) {
            return null;
        }
        YandexCountdownButton overlay = new YandexCountdownButton(hostButton.getContext());
        overlay.setId(View.generateViewId());
        overlay.setScaleType(ImageView.ScaleType.CENTER_INSIDE);
        overlay.setImageResource(ResourceUtils.getIdentifierOrThrow(
        ...
        // The invisible host anchor still reserves its slot and receives host
        // geometry updates; the overlay is the visible and interactive layer.
        hostButton.setImageAlpha(0);
        overlay.getViewTreeObserver().addOnPreDrawListener(() -> {
            syncCountdownOverlay(hostButton, overlay);
            return true;
        });
        syncCountdownOverlay(hostButton, overlay);
        return overlay;
    ...
    private static void syncCountdownOverlay(
            ImageView hostButton,
            YandexCountdownButton overlay
    ) {
        if (!hostButton.isAttachedToWindow() || !overlay.isAttachedToWindow()) return;

        ViewGroup.LayoutParams hostParams = hostButton.getLayoutParams();
        if (hostParams != null && overlay.getLayoutParams() != hostParams) {
            overlay.setLayoutParams(hostParams);
        }
        if (overlay.getPaddingLeft() != hostButton.getPaddingLeft()
                || overlay.getPaddingTop() != hostButton.getPaddingTop()
                || overlay.getPaddingRight() != hostButton.getPaddingRight()
                || overlay.getPaddingBottom() != hostButton.getPaddingBottom()) {
            overlay.setPadding(
                    hostButton.getPaddingLeft(),
                    hostButton.getPaddingTop(),
                    hostButton.getPaddingRight(),
                    hostButton.getPaddingBottom()
            );
        }
        if (overlay.getX() != hostButton.getX()) overlay.setX(hostButton.getX());
        if (overlay.getY() != hostButton.getY()) overlay.setY(hostButton.getY());
        if (overlay.getAlpha() != hostButton.getAlpha()) overlay.setAlpha(hostButton.getAlpha());
        if (overlay.getVisibility() != hostButton.getVisibility()) {
            overlay.setVisibility(hostButton.getVisibility());
        }

        Drawable background = hostButton.getBackground();
        Drawable.ConstantState state = background != null ? background.getConstantState() : null;
        if (overlay.backgroundSnapshot != state) {
            overlay.setBackground(state != null ? state.newDrawable().mutate() : background);
            overlay.backgroundSnapshot = state;
        }
    }
